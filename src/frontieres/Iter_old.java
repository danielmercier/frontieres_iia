...
public class Iter_old extends TimerTask implements AlgoFrontieres {
	...
	public Iter_old(HeuristiqueFrontieres heuristique) {
	...
	public CoupFrontieres meilleurCoup(int t, PlateauFrontieres board) {
		timeLimit = t;
		leaves = nodes = 0;
		orderedMoves.clear();
		...
		while(elapsed < timeLimit) {
			...
			// ATTENTION : problème si TIE vaut zéro (càd une valeur possible pour une configuration non-TIE)
			// trouver une valeur pour TIE qui soit toujours > coefPrises et < max(DiffAvancee) ?
			// → mettre TIE en non final, et dans les setters des coefs, lancer un recalcul de sa valeur
			// OU ALORS : enlever de la condition
			++ profMax;
		}
		showStat();
